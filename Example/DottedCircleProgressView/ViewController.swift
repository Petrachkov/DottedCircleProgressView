...
class ViewController: UIViewController {
	...
	override func viewDidLoad() {
		...
		let progressConfig  = ProgressLayerConfigurator(fillColor : UIColor.white.cgColor,
		                                               ...
		                                               frame : CGRect(x: 9, y: 9, width: frame.width - 18, height: frame.height - 18),
		                                               instanceCount : 8,
		                                               dotSize : 3);

		progressView = DottedCircleProgressView(frame: frame, progressConfigurator: progressConfig);
		progressView.backgroundColor = UIColor(red: 0, green: 0, blue: 0, alpha: 0.6);
		progressView.layer.cornerRadius = progressView.frame.size.width * 0.5;
		...
		let visibilitySwitch = UISwitch(frame: CGRect(x: self.view.frame.width * 0.5 - 50 * 0.5,
		                                              ...
		                                              width: 60,
		                                              height: 40));
		visibilitySwitch.addTarget(self, action: #selector(ViewController.switchStateChanged(sender:)), for: .valueChanged);
